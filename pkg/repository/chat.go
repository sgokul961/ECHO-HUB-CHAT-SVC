...
func NewChatRepository(db *mongo.Database) interfaceR.ChatRepository {
	return &ChatRepository{ChatCollection: db.Collection("chats"), MessageCollection: db.Collection("messages")}
}

func (c *ChatRepository) CreateChatRoom(user1, user2 int64) error {
...
func (c *ChatRepository) IsChatExist(user1, user2 int64) (bool, error) {
	filter := bson.M{
		"users": bson.M{"$all": []int64{user1, user2}},
	}
...
func (c *ChatRepository) GetAllChats(id int64) ([]models.Chat, error) {
	// Define the filter and projection
	filter := bson.M{"users": bson.M{"$in": []int64{id}}}
...
func (c *ChatRepository) FetchRecipient(chatId primitive.ObjectID, userId int64) (int64, error) { //<-------------------changed
	filter := bson.M{"_id": chatId}
